...
**状態:** 1〜4 完了（2026-09-08）。`d15f3e4` `quote`、次のコミットで `strikeout`。
残っているのは下の「まだ決まっていないこと」だけ。
...
**`Superscript` と `Subscript` は今も `@mark` に潰れている。**

`from_pandoc.rs` に残した唯一の意図的な嘘。small caps と違って**意味を運ぶ** ―
`x²` と `H₂O` は `x2` と `H2O` と同じ語ではない ― ので、素の内容に落とすのも
無料ではない。要素を作るのが筋に見えるが、strikeout のときの判断
（意味を運ぶなら作る）をそのまま適用すると 2 つ増えることになるので、
作者の決定を待つ。

**`format.quote.always_newline` はどこからも読まれていない。**

`tomet-config` の `apply_entry` に該当する分岐が無い。`d15f3e4` で名前だけ
`blockquote` から変えたが、依然として死んでいる。実装するか、消すか。

**`apps/web/static/app.js` は古い名前のまま。**

ミニファイされたビルド成果物なので手で直していない。中に `tomet-blockquote` と
`b(o)==="blockquote"` が残っている。ソース側を直して再ビルドが要る。
