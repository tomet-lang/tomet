 ...
 * @returns {any}
 */
export function highlightSpans(source) {
    const ptr0 = passStringToWasm0(source, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.highlightSpans(ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
export function parseValue(source) {
    const ptr0 = passStringToWasm0(source, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.parseValue(ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
 ...
 * @returns {string}
 */
export function printDocument(doc_val) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.printDocument(doc_val);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
 ...
 * @returns {string}
 */
export function toMarkdown(source_or_doc) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.toMarkdown(source_or_doc);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('tomet_js_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
