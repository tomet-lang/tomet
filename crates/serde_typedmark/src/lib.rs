//! `serde` support for TypedMark's data subset (`typedmark_ast::Value`):
...
//!
//! Headings, prose, and links (the rest of `typedmark_ast::Document`) have
//! no serde equivalent and aren't handled here; `to_string`/`from_str`
//! only ever read or write a data-only `.tm` document.
