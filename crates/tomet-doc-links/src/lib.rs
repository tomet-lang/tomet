//! Broken-link checking for a vault of `.tmt`/`.tmt` files: extracts every
...
//! to `tomet-validator` as a separate lint), caches the extraction
...
//! for link-checking, analogous to `tomet-doc-resolver`'s role for
