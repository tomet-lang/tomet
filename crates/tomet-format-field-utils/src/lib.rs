...
//! `tomet-printer` (which still owns the `Document`-mutating
//! `ensure_document_id_with_config` and the `.tmt`-syntax rendering that
//! call these) so `tomet-formatter` can reuse the same
