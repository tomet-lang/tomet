...
2. **Validation Rules**:
   - **Duplicate ID Detection (`id.rs`)**: Walks all element nodes (headings, typed elements, bare elements, inline elements) via `tomet_tree::walk_document` and checks for duplicate `id` attributes in both `{id:...}` and `(id:...)`.
