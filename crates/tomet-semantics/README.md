...
2. **Core Capabilities**:
   - **Element Classification (`kind.rs`)**: Canonical recognition of Tomet's built-in vocabulary (`@meta`, `@config`, `@links`, `@link`, `<embed>`, `hr`, `em`, `strong`, `mark`, `codeblock`, `blockquote`, `table`, `heading`, `ol`, `ul`, `bare`, `interp`) via `ElementKind` and `classify(&Element) -> ElementKind`.
