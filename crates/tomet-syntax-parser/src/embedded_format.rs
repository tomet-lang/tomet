...
//! to the corresponding crate's own parser rather than Tomet's
...
/// Tomet: only brace depth and quoted strings are tracked, so the real
...
mod tests {
    ...
    use tomet_ast::{Block, ElementValue, Sigil, Value};
    ...
    fn config_sets_the_default_format_for_later_elements() {
        ...
        // parsed as real JSON (invalid as Tomet's lightweight grammar).
