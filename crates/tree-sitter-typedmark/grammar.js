...
module.exports = grammar({
  ...
  rules: {
    ...
    paragraph: ($) => prec.right(repeat1(seq(repeat1($._line_item), $._newline))),

    // ---- inline content ---------------------------------------------
    _line_item: ($) =>
      choice($.text, $.code_span, $.emphasis, $.strong, $.mark, $.element, $.punctuation),
    _bracket_item: ($) =>
      choice($._line_item, $._newline),
    ...
    type_element: ($) =>
      seq("<", field("name", $._element_name), ">", prec.right(2, repeat($._element_group))),
    at_element: ($) =>
      seq("@", optional(field("name", $._element_name)), prec.right(2, repeat($._element_group))),
    _element_group: ($) =>
      choice($.input_group, $.area_group, $.value_group),
    ...
    input_group: ($) =>
      seq(token(prec(1, "(")), optional(seq(optional($._blank_gap), $.value)), optional($._blank_gap), ")"),
    ...
    children: ($) => prec.right(repeat1(seq(optional($._blank_gap), $.bare_element))),
    ...
    _element_name: ($) => alias(token(prec(1, /[A-Za-z_][A-Za-z0-9_.\-]*/)), $.identifier),
    ...
    // token of lookahead).
    map: ($) => prec.right(seq($.map_entry, repeat(seq($._entry_sep, $.map_entry)))),
    ...
    map_entry: ($) =>
      seq(
        field("key", alias(token(prec(1, /[A-Za-z_][A-Za-z0-9_.\-]*/)), $.identifier)),
        ":",
        field("value", $._entry_value),
      ),
    _entry_value: ($) => choice($.seq, $.string, $.braced_map, $._value_scalar),
    braced_map: ($) =>
      seq("{", optional(seq(optional($._blank_gap), $.map)), optional($._blank_gap), "}"),
    seq: ($) =>
      seq(
        "[",
        optional(
