...
use typedmark_ast::{Document, InterpExpr, InterpExprKind, Literal, Value};
...
mod tests {
    ...
    fn interp(src: &str) -> InterpExpr {
        ...
        match &doc.blocks[0] {
            typedmark_ast::Block::Element(el) => match &el.value {
                Some(typedmark_ast::ElementValue::Interp(expr)) => expr.clone(),
