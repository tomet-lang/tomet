//! Directory scanning and read-only metadata cataloging for `.tm`/`.tmt`
//! files. Split out of `typedmark-tui`'s `engine::batch_meta` module so
...
/// Collects every `.tm`/`.tmt` file under `path` (or just `path` itself
    ...
    F: FnMut(&Element),
{
    ...
    let _ = typedmark_walker::walk_document(doc, &mut ElementVisitor(&mut f));
}
...
"#;
    ...
    fn test_collect_tm_files_finds_tm_and_tmt_only() {
        ...
        let names: Vec<_> = files
            ...
            .collect();
        assert_eq!(names, vec!["a.tm", "b.tmt"]);
