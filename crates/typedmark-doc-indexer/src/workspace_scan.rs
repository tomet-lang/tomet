...
//! `typedmark-tui`'s Explorer tree, Migration candidate list, and BatchMeta
...
//! `typedmark-tui`, keyed by path, reconciled against this index's current
    ...
    pub fn migration_candidates(&self) -> Vec<MigrationCandidate> {
        self.entries
            ...
            .map(|(p, _)| MigrationCandidate {
                source_path: p.clone(),
                target_path: p.with_extension("tm"),
            })
            .collect()
    }

    /// Every currently-cataloged `.tm`/`.tmt` file.
    pub fn meta_paths(&self) -> Vec<PathBuf> {
        self.entries
            .iter()
            .filter(|(_, k)| **k == EntryKind::TypedMark)
            .map(|(p, _)| p.clone())
            .collect()
    }
}
...
mod tests {
    ...
    fn refresh_path_adds_updates_and_removes() {
        ...
        fs::write(dir.join("existing.tm"), "@meta{}\n").unwrap();
