...
impl LinkCache {
    ...
    pub fn links_for(&mut self, source: &Path) -> Result<CacheOutcome, LinkCacheError> {
        ...
        let doc = typedmark_parser::parse_document(&src).map_err(|e| LinkCacheError::Parse {
            path: path_str.clone(),
            source: e,
        })?;
    ...
    fn read_links(&self, path_str: &str) -> rusqlite::Result<Vec<DocumentLink>> {
        ...
        for row in rows {
            ...
            links.push(DocumentLink {
                kind,
                target,
                span: typedmark_ast::Span::new(
                    typedmark_ast::Position::new(start_line as usize, start_col as usize, 0),
                    typedmark_ast::Position::new(end_line as usize, end_col as usize, 0),
                ),
            });
        }
        Ok(links)
