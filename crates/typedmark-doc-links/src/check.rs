...
/// Enumerates `.tm`/`.tmt` files under `root`, pulls each one's links
...
pub fn check_vault(
    ...
    let files = typedmark_indexer::collect_tm_files_with_config(root, config, config_root);
    let existing = typedmark_indexer::collect_all_paths_with_config(root, config, config_root);
...
mod tests {
    ...
    fn dot_slash_prefix_resolves_relative_to_source_dir() {
        let source = Path::new("/vault/notes/nested.tm");
    ...
    fn ref_target_matches_by_stem_or_full_filename() {
        ...
        existing.insert(PathBuf::from("/vault/notes/real.tm"));

        assert!(resolve_ref_target("real", &existing));
        assert!(resolve_ref_target("real.tm", &existing));
    ...
    fn tm_target_strips_fragment_before_resolving() {
        let source = Path::new("/vault/notes/nested.tm");
        let root = Path::new("/vault");
        let path_part = "docs/asdf.tm#some-id".split('#').next().unwrap();
        assert_eq!(
            resolve_file_target(source, path_part, root),
            PathBuf::from("/vault/docs/asdf.tm")
        );
    }
