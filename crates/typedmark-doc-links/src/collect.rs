...
use typedmark_ast::{Document, Element, Span};
use typedmark_semantics::{ElementKind, TargetScheme, link_target_of, target_scheme};
use typedmark_walker::{Visitor, walk_document};
...
"#;
        let doc = typedmark_parser::parse_document(src).unwrap();
