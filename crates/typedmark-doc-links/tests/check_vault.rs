...
use typedmark_links::{LinkCache, check_vault};
...
fn finds_broken_links_and_ignores_external_urls() {
    ...
    assert_eq!(
        ...
        "expected good.tm + bad.tm + sub/nested.tm"
    );
    let mut broken_targets: Vec<&str> = report.broken.iter().map(|b| b.target.as_str()).collect();
    broken_targets.sort();
    assert_eq!(
        broken_targets,
        vec!["missing.tm#anchor", "missing.txt", "no-such-note"],
        "full report: {:?}",
        report.broken
    );
...
fn resolves_source_relative_project_root_relative_and_ref_targets() {
    ...
    let mut broken_from_nested: Vec<&str> = report
        .broken
        .iter()
        .filter(|b| b.source.ends_with("nested.tm"))
    ...
    // stem search against `real.txt`; `tm:good.tm#anchor` resolves
    // (fragment stripped, `good.tm` exists at the project root); only
    // `ref:no-such-note` and `tm:missing.tm#anchor` are actually broken.
    assert_eq!(
        broken_from_nested,
        vec!["missing.tm#anchor", "no-such-note"]
    );
...
fn second_run_reuses_the_cache_without_reparsing() {
    ...
    for file in typedmark_indexer::collect_tm_files(&root) {
