...
//! `typedmark-ast`'s node definitions, not to any one consumer's business
...
//! `Element{ sigil: Sigil::Bare, .. }` (see `typedmark_ast::Element::list_item`),
//! so there's no separate id-bearing node kind to special-case here
//! anymore.

use std::ops::ControlFlow;
use typedmark_ast::{Block, Document, Element, ElementValue, Inline, Value};

/// Merges an [`Element`]'s `args` and `value` (when `value` is
/// `ElementValue::Data(Value::Map(_))`) into one attrs view -- `value`'s
/// keys win on conflict. Mirrors `typedmark-doc-resolver::interp::node_value`,
...
mod tests {
    ...
    fn visits_every_node_kind_in_document_order() {
        ...
        // `typedmark-semantics::classify`); a list item is `Element`
    ...
    fn walk_document_mut_visits_blocks_nested_under_a_list_item() {
        ...
        // `typedmark-parser::list::parse_list_internal`.
        let doc = parse("- outer\n  - <x>()\n");
