...
/// Reads `path`, parses it as TypedMark, and returns the `Value` held by
...
pub fn resolve_settings_file(path: &Path) -> Result<Value, ResolveError> {
    ...
    let doc = typedmark_parser::parse_document(&src).map_err(|source| ResolveError::Parse {
        path: path.to_path_buf(),
        source,
    })?;
    settings_block(&doc)
...
/// `docs/ja/specifications/builtin-args.tm`), then resolved via
...
/// blank line between them (exactly how `docs/docs.settings.tm` is
/// written) used to get folded into one `Block::Paragraph` by the
/// parser's lazy-continuation rule. `typedmark-parser` no longer does that
...
/// `typedmark-parser` normally produces for a `{value}`-only element, but
...
mod tests {
    ...
    fn resolves_a_valid_settings_file() {
        let value = resolve_settings_file(&fixture("valid_settings.tm")).unwrap();
        match value {
            Value::Map(entries) => {
    ...
    fn missing_file_is_an_io_error() {
        let err = resolve_settings_file(&fixture("does_not_exist.tm")).unwrap_err();
        assert!(matches!(err, ResolveError::Io { .. }), "got {err:?}");
    }

    #[test]
    fn unparseable_file_is_a_parse_error() {
        let err = resolve_settings_file(&fixture("invalid_syntax.tm")).unwrap_err();
        assert!(matches!(err, ResolveError::Parse { .. }), "got {err:?}");
    }

    #[test]
    fn file_with_no_settings_block_is_reported() {
        let err = resolve_settings_file(&fixture("no_settings_block.tm")).unwrap_err();
        assert!(
    ...
    fn settings_file_ref_extracts_the_path() {
        ...
        el.args = Some(Value::Map(vec![(
            "file".to_string(),
            Value::String("docs/docs.settings.tm".to_string()),
        )]));
        assert_eq!(settings_file_ref(&el), Some("docs/docs.settings.tm"));
    }
    ...
    fn resolve_settings_ref_joins_against_project_root() {
        ...
        el.args = Some(Value::Map(vec![(
            "file".to_string(),
            Value::String("valid_settings.tm".to_string()),
        )]));
