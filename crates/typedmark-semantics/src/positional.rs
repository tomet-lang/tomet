...
/// `typedmark_ast::Element::list_item`). `Sigil::Bare` has no name to
...
/// Sentinel key `typedmark_parser` tags a bare, unkeyed positional entry
/// with (`(a, b)`'s two entries, or `@link(tm:foo, depends_on)`'s second
/// one) -- must match `typedmark_syntax_parser::value::POSITIONAL_ENTRY_KEY`
/// exactly. Duplicated here rather than imported since `typedmark-semantics`
/// deliberately doesn't depend on `typedmark-parser` (see this crate's own
...
mod tests {
    ...
    fn does_not_recover_for_non_link_embed_elements() {
        ...
        let mut el = Element::new(Sigil::Type("caution".to_string()));
        el.args = Some(Value::Map(vec![(
            "tm".to_string(),
