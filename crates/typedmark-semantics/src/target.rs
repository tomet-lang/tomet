...
//! extraction, adopted by `typedmark-codegen-html`/`typedmark-codegen-markdown`/
//! `typedmark-doc-links` instead of each reading `el.args` raw.
...
pub enum TargetScheme {
    ...
    File,
    /// `tm:path/to/doc` -- another TypedMark document, project-root-relative
