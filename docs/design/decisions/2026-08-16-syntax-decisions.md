...
1. **`[]` collision with lists**: `parse_list` in
   `crates/tomet-parser/src/document.rs` reads each list item's
   ...
   syntax explored in `docs/develop/idea.tmt` was not adopted; multi-line
...
2. **Dual link notation**: `tomet-semantics::ElementKind` has no
   ...
   element with `args`/`content` reordered (see `docs/spec/
   syntax.tmt`'s `#[ 柔軟性 ]` section), not two competing constructs.

`docs/develop/idea.tmt` still carried the pre-decision brainstorming for
