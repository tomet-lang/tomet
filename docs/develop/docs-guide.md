...
written in, and which files are generated. `docs/README.tmt` is the map;
...
| instructional -- it shows how to get something done | `guide/` | link it from `docs/README.tmt` if it is an entry point |
...
| a statement of what Tomet is and why it exists | `fact/` | Japanese, `.tmt`; it is a declaration, not a dated record |
...
`docs/README.tmt` is the map. It is a Tomet document like the rest of
`spec/`, `guide/`, `examples/` and `fact/`, so it is subject to
`just docs-check` and doubles as dogfooding. No `.md` is generated from
it: `tomet export` currently escapes the backticks in prose
(`` `spec/` `` becomes ``\`spec/\``) and emits a `<div>` for the
`@settings` header, so the rendering would be worse than the source.
The consequence is that GitHub shows no rendered index for the `docs/`
directory -- fix those two export bugs and this can become a generated
`.md` like `spec/` and `guide/`.
