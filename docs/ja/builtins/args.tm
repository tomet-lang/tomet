...
file: <file>                   // @(file:...) / @file(file:...) として推論・特別レンダリングされる。
                                // プロジェクトルート相対で、任意のファイル/添付を指す
tm: <tm>                       // @(tm:...) / @tm(tm:...) として推論・特別レンダリングされる。
                                // file: と同じくプロジェクトルート相対だが、他の .tm/.tmt ドキュメント
                                // を指す点で意味的に区別される。#fragment を併記して文書内の特定の id
                                // を指せる (tm:path/to/doc#some-id)
id: <id>                        // @(id:...) / @id(id:...) として推論・特別レンダリングされる。
                                // idベースの参照。現状は同一ドキュメント内のみ (`${id}`と同じid属性空間)
ref: <ref>                     // @(ref:...) / @ref(ref:...) として推論・特別レンダリングされる。
                                // ファイル名/タイトルでプロジェクト内を検索して解決する
                                // (旧 wiki: キーの役割を引き継いだもの)
