...
- `@[Wiki](url:https://example.com)`  // Same meaning, different order
...
- `[]` (`content-span`) is valid only as a group immediately following `<T>`, `@`, or `#` (heading). A standalone `[...]` at the beginning of a line is not a content-span; it is simply a string.
  → Therefore, do not write a bare word followed by `[]`, such as `Caution [ ... ]`. Instead, explicitly specify the type: `<caution>[ ... ]`.
  → List markers `-` do not use `[` or `]` in the first place, so there is no conflict.
...
- If `(format)` inside `{...}` contains a `format` key (`json` / `yaml` / `toml`), the contents are parsed directly as actual JSON/YAML/TOML source using the `serde_json` / `serde_yaml` / `toml` crates, rather than TypedMark's lightweight `Value` syntax (decided and implemented).
