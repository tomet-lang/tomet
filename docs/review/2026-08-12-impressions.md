...
2. **Embedded Format Interoperability (`format: json/yaml/toml`)**
   - `typedmark-parser::embedded_format` natively allows embedded JSON, YAML, or TOML inside `{value}` blocks via `format: json|yaml|toml`. This provides a pragmatic escape hatch for complex schemas without overloading the markup language itself.

3. **Well-Structured Ecosystem Architecture**
   - Clear separation of concerns across dedicated workspace crates: `typedmark-ast`, `typedmark-lexar`, `typedmark-parser`, `typedmark-renderer`, `typedmark-markdown`, `serde_typedmark`, `typedmark-formatter`, and `tree-sitter-typedmark`.

4. **Grammar Drift Protection & Test Harness**
   - Explicit test harness (`*_fixture_has_only_known_error_cases`) in `tree-sitter-typedmark` detects drift between the canonical hand-written parser (`typedmark-parser`) and the tree-sitter grammar (`grammar.js`).
...
2. **Identity & Scope Boundaries**
   - Pushing TypedMark to simultaneously serve as a prose markup, a JSON replacement, and an in-parser dynamic template engine (e.g., `$add(a, b)` or `${a % b}` proposed in design notes) risks creating a "jack of all trades, master of none".
   - Dynamic evaluation inside `typedmark-parser` would introduce security risks, parser performance degradation, and static analysis complexity.

3. **AST Position Tracking Constraints**
   - `typedmark_ast::Document` currently does not carry source span/position info (`Line`, `Column`, byte offset).
   - *Impact*: `typedmark-formatter` is limited to text-level whitespace normalization rather than true AST-driven lossless formatting. LSP capabilities (diagnostics ranges, Go-to-Definition) are also constrained.
...
- **Keep Parser Pure**: Keep `typedmark-parser` strictly deterministic, fast, and static. Do not build an expression evaluator inside the core parser; handle templating in a separate preprocessing layer if needed.
...
- **Add `Span` to AST Nodes**: Attach byte offset and line/column ranges to all AST nodes in `typedmark-ast`.
- **Lossless Formatter & Rich LSP**: Upgrade `typedmark-formatter` to perform AST-driven lossless formatting and enable precise LSP diagnostic ranges.

### Pillar 4: Grammar Freeze & Targeted Toolchain Execution
- **Freeze Core Grammar**: Finalize the core grammar specification and synchronize `typedmark-parser` and `tree-sitter-typedmark`.
- **Focus Core Engineering**: Prioritize `apps/typedmark` (CLI) and `apps/typedmark-lsp`, keeping placeholder binding stubs paused until core 1.0 stabilization.
