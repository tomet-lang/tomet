...
after the core grammar freeze. `docs/develop/idea.tmt` (lines 8, 124-134)
...
1. **Unified `${...}` sigil, not a competing `$func(...)` form.** A bare
   ...
   `crates/tomet-parser/src/document.rs`.
...
2. **`${...}` reuses `Element`, not a new `Inline` variant.**
   ...
   match over `Inline` across `tomet-html`/`tomet-markdown`/the
   ...
   its own `InterpExpr` type (`crates/tomet-ast/src/lib.rs`) rather
...
3. **`InterpExpr` is `{ kind: InterpExprKind, span: Span }`, and every
   ...
   other `tomet-ast` node already uses (`Text`, `Heading`, `Element`,
   ...
   in `crates/tomet-parser/src/lib.rs` for the composed cases. No
   infix operators yet -- `${a + b}`/`${a % b}` (from `idea.tmt`'s
...
4. **No newlines inside `${...}`.** `parse_dollar_element` only skips
   ...
   `crates/tomet-parser/src/lib.rs`.

5. **Evaluation is out of scope for this decision.** `InterpExpr` is an
   unresolved syntax tree; looking up an `Identifier`/`Member`'s
   referenced element (`tomet-resolve`'s job) or calling a `Call`'s
   named function (`tomet-compute`'s job) is separate, not-yet-built
   work. Both crates currently have no code that consumes `Element`/
   `ElementValue::Interp`. `tomet-html`/`tomet-markdown`/the TUI
   printer each independently re-render an unresolved `${...}` back to
   display/source text in the meantime (not shared via `tomet-ast`
...
`docs/develop/idea.tmt` trimmed to point at this doc for the settled
