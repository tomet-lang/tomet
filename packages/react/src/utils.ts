import type { Element, ElementValue, Inline, Sigil, Value } from './types';
...
/**
 * Extracts target URL/href from element arguments for link-shaped elements (@link, <link>).
 */
...
/**
 * Helper to safely extract Data value from ElementValue.
 */
export function getElementDataValue(value: ElementValue | null | undefined): Value | null {
  if (!value) return null;
  if ('Data' in value) return value.Data;
  if ('type' in value && value.type === 'Data') return value.data;
  return null;
}
