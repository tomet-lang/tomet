...
const CASES: &[Case] = &[
    ...
    case(None, "要素の行で開いてもよい", "@blockquote| 本文\n"),
    case(
        None,
        "リスト項目も例外ではない。継続行は開いた `|` と同じ列に立つ",
        "- (x)| 一行目\n     | 二行目\n",
    ),
    case(None, "見出しも同じ", "#| 一行目\n | 二行目\n"),
    case(
        None,
        "継続する先が無い `|` は地の文。地の文にエスケープが無いので、\
         行頭の `|` は綴れなければならない",
        "| a | b |\n",
    ),
    // ---- lists ------------------------------------------------------
    ...
    case(
        None,
        "`(...)` だけの項目。本文が続かなくてもマーカーになる。\
         `#(id: a)` が引数つきの見出しになるのと同じ",
        "- ( )\n-(x)\n- (y)\n",
    ),
    case(
        None,
